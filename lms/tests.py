...
class LessonTestCase(APITestCase):
    ...
    def test_user_subscribe(self):
        login = self.client.post(f"{self.BASE_URL_API}/users/login/",
                                 data={'email': '1@1.ru', 'password': '1234'})
        access = login.json()['access']
        data = {"course_id": 2}
        headers = {"Authorization": f"Bearer {access}"}
        response = self.client.post(f"{self.BASE_URL_API}/users/subscribe/", data=data, headers=headers)
    ...
    def test_crud_lesson(self):
        ...
        assert response.json() == {
            ...
            "next": None,
            "previous": None,
            "results": [{"id": 1, "name": "Test Lesson", "description": None, "preview": None, "video_url": None}],
        }
        # detail lesson
        ...
        response = self.client.post(
            f"{self.BASE_URL_API}/lms/lessons/",
            {"name": "test2", "course": 1, "owner": self.user, "video_url": "www.fsfsf.com"},
        )
        assert response.json() == {"video_url": ["Ссылка должна вести на ресурсы: 'youtube.com'"]}
        # create lesson - valid url
        response = self.client.post(
            f"{self.BASE_URL_API}/lms/lessons/",
            {"name": "test2", "course": 1, "owner": self.user, "video_url": "www.youtube.com"},
        )
        ...
        response = self.client.put(
            f"{self.BASE_URL_API}/lms/lessons/2/", {"name": "test_test", "video_url": "www.youtube.com"}
        )
        assert response.json() == {
            "id": 2,
            "name": "test_test",
